...
import { CURSOR_LIMITS, DEFAULT_SETTINGS } from '@shared/constants';
import type { AppSettings, CursorAuthState, CursorLoginPhase } from '@shared/types';
import { logger } from '../../logger';
import type { SettingsManager } from '../SettingsManager';
import { SecretStore, CURSOR_API_KEY_SECRET } from '../../secrets/SecretStore';
import { redactCursor, resolveCursorExecutable, runCursorAgent, spawnCursorLogin } from './exec';
  ...
  private async doProbe(force: boolean): Promise<CursorAuthState> {
    ...
    if (!exe) {
      this.setState({
        ...
        error:
          'Cursor CLI (cursor-agent) not found on PATH. Install it from cursor.com/docs, then hit Refresh.',
      });
      return this.state;
    }
...
function parseStatus(stdout: string): { authenticated: boolean; account?: { email?: string; name?: string } } | null {
  try {
    ...
    const email = str(obj.email) ?? str((obj.account as Record<string, unknown> | undefined)?.email);
    const name = str(obj.name) ?? str((obj.account as Record<string, unknown> | undefined)?.name);
    const authenticated =
